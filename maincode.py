...
# pip install langdetect, google_trans_new, speechrecognition, requests, transformers torch senterpiece, pyaudio.
...
def analyse_sentiments(detected_language):
    ...
    print("Do you want the sentiment analysis of this sentence?")
    print("Y/N")
    y_n = input()
    y_n = y_n.lower()
    if y_n == 'y':
        my_model = pipeline('sentiment-analysis')
        print(my_model(detect_language))
        to_continue_further()
    elif y_n == 'n':
        print("Thank you! ")
        to_continue_further()
...
def main_function():
    ...
    if choice_of_input.human_text == 'text':
        ...
        if ask_user_about_lang_pref.pref == 'english':
            ...
            analyse_sentiments(detect_language)
